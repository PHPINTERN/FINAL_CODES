...
if (isset($_GET["email"]) && $_GET["email"] != '' &&isset($_GET['password']) && $_GET['password'] != '') {
    ...
    if(strlen($password)<=8) {
        $flag--;
        $resp1["message"][] = "Password must contain more then 8 character.";
    }
    if (!preg_match('/[A-Z]/', $password)) {
        $flag--;
        $resp1["message"][]= "Password must contain at least one Uowercase letter.";
    }
    // Check for at least one lowercase letter
    if (!preg_match('/[a-z]/', $password)) {
        $flag--;
        $resp1["message"][]= "Password must contain at least one lowercase letter.";
    }

    // Check for at least one special character
    if (!preg_match('/[@_]/', $password)) {
        $flag--;
        $resp1["message"][] = "Password must contain at least one Special letter (only @ and _ ).";
    }
    if (preg_match('/[ ]/', $password)) {
        $flag--;
        $resp1["message"][] = "Password should not contain any space";
    }
    if (!preg_match('/[0-9]/', $password)) {
        $flag--;
        $resp1["message"][] = "Password must contain at least one number character.";
    }
